...
class PaymentTracking(BaseModel):
    ...
    def recalc_pending(self):
        # pending = approved_budget + additional_amount - payout - sum(milestones paid)
...
class PaymentMilestone(BaseModel):
    payment_tracking = models.ForeignKey(PaymentTracking, on_delete=models.CASCADE, related_name='milestones')
...
class FinanceTransaction(BaseModel):
    ...
    description = models.TextField(blank=True, null=True)
    amount = models.DecimalField(max_digits=12, decimal_places=2)
    related_payment = models.ForeignKey(PaymentTracking, on_delete=models.SET_NULL, null=True, blank=True, related_name='transactions')
...
class ProjectBudget(models.Model):
    ...
    project = models.ForeignKey(
        ...
        related_name="project_creation_project_budgets"  )
